...
import type { Bytes, Ciphertext, Identity, PublicIdentity, UserId, Crypto, CiphertextHeader } from "./interfaces.ts";

const cache = new WeakMap<Crypto, Constructors>();

type Constructors = ReturnType<typeof defConstructors>;

function defConstructors(crypto: Crypto) {
...
export function useConstructors(crypto: Crypto): Constructors {
    return cache.get(crypto) ?? defConstructors(crypto);
}