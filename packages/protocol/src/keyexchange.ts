...
export class KeyExchangeManagerConstructor implements KeyExchangeManager {
    ...
    public async processPreKeyBundle(bundle: PreKeyBundle, associatedData?: Bytes): Promise<{ session: Session, message: PreKeyMessage }> {
        ...
        const encrypted = session.encrypt(this.crypto.Utils.concatBytes(this.crypto.hash(this.publicIdentity.bytes), this.crypto.hash(identityKey.bytes), associatedData ?? new Uint8Array()));
        if (!encrypted)
    ...
    public async processPreKeyMessage(message: PreKeyMessage): Promise<{ session: Session, associatedData?: Bytes }> {
        const { PublicIdentityConstructor, UserIdConstructor } = useConstructors(this.crypto);
