...
import type { Crypto, Bytes, InitialSessionState, KeyChainState, KeyStore, Session, SessionManager, Ciphertext, UserId, DecryptResult } from "./interfaces.ts";
...
export class SessionManagerConstructor implements SessionManager {
    ...
    private async getUserSession(userId: UserId | string): Promise<Session> {
    ...
    private async getSession(sessionTag: string): Promise<Session> {
...
export class SessionConstructor implements Session {
    ...
    public async save(): Promise<void> {
        ...
        if (this.#receivingChain?.headerKey)
            await this.keyStore.setSessionTag(this.crypto.hash(this.#receivingChain.headerKey), this.sessionTag);
        if (this.#receivingChain?.nextHeaderKey)
            await this.keyStore.setSessionTag(this.crypto.hash(this.#receivingChain.nextHeaderKey), this.sessionTag);
